...
/**
 * Disconnect and clean up HTTP connections.
 */
RestAdapter.prototype.disconnect = function() {
  // Clean up any HTTP client connections
  if (this._httpClient && typeof this._httpClient.destroy === 'function') {
...
RestAdapter.prototype.createHandler = function() {
  ...
  // Express 5 defaults to the 'simple' query parser, which does not understand
  // nested syntax like `?a[0]=x&a[1]=y` or `?obj[k]=v`. Re-parse the raw query
  // string with `qs` so the REST adapter receives objects/arrays as expected.
  // `req.query` is a getter in Express 5, so override it via defineProperty.
  root.use(function(req, res, next) {
    const queryIndex = req.url.indexOf('?');
    const parsed = queryIndex >= 0 ? qs.parse(req.url.slice(queryIndex + 1)) : {};
...
RestAdapter.prototype._invokeMethod = function(ctx, method, next) {
  ...
  // Execute steps sequentially using native async/await
  ...
  // Execute the steps and handle completion
  executeSteps()
    .then(() => {
