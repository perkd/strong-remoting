...
function createRequestCompatibleInterface() {
  ...
  function request(options, callback) {
    const result = httpClient.request(options, callback);
    ...
    // For callback mode, return an object with pipe method for stream compatibility
    return {
      pipe: function(destination) {
        // Create streaming request
        const streamOptions = typeof options === 'string' ? { url: options } : { ...options };
        streamOptions.responseType = 'stream';

        httpClient._makeAxiosRequest(streamOptions)
          .then(response => {
