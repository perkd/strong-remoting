...
function SocketIOContext(req, ctorArgs, args, method, typeRegistry) {
  // Call ContextBase constructor if method and typeRegistry are provided
  if (method && typeRegistry) {
    ContextBase.call(this, method, typeRegistry);
  } else {
    // Fallback to EventEmitter for backward compatibility
  ...
  // Set method if provided but not set by ContextBase
...
/**
 * Inherit from `ContextBase` (which inherits from EventEmitter).
 */

inherits(SocketIOContext, ContextBase);
