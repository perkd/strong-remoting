...
ContextBase.prototype.invoke = function(scope, method, fn) {
  // Basic invoke implementation for ContextBase
  // Subclasses should override this method for specific behavior
  const args = this.args || [];
  const remotingOptions = {};
  const ctx = this;

  // Invoke the shared method with proper signature
  method.invoke(scope, args, remotingOptions, ctx, function(err, result) {
    if (err) {
      return fn(err);
    }
    fn(null, result);
