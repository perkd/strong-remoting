  ...
  return function wrappedInvoke(method, ctorArgs, args, callback) {
    ...
    if (actualCallback) {
      ...
      return new Promise((resolve, reject) => {
        ...
        // Use _original_invoke property if available (for testing), otherwise use closure variable
        const contextObj = context || this;
        const methodToCall = (contextObj && contextObj._original_invoke) || originalInvoke;
        methodToCall.apply(contextObj, callbackArgs);
      });
    }
  };
}
