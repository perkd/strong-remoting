...
HttpContext.prototype.invoke = function(scope, method, fn, isCtor) {
  ...
  if (pipeDest) {
    ...
    const returnStreamDesc =
      method.streams && method.streams.returns;

    new Promise((resolve, reject) => {
      method.invoke(scope, args, self.options, ctx, function(err, result) {
        if (err) {
          ...
          // Streams resolve through a Promise microtask, but a stream may
          // emit 'error' synchronously or via process.nextTick before that
          // microtask runs. Capture it on the stream itself so the response
          // handler can replay it instead of crashing the process.
          if (returnStreamDesc && result) {
            const stream = result[returnStreamDesc.arg];
            if (stream && typeof stream.once === 'function') {
              stream.once('error', (e) => { stream._earlyError = e; });
            }
          }
          resolve(result);
        }
...
HttpContext.prototype.shouldReturnEventStream = function() {
  ...
  // Only return an event stream when the client explicitly opts in. Using
  // req.accepts() alone is too eager: when the client sends no Accept header
  // (or `*/*`), it matches `text/event-stream` and accidentally switches to
  // SSE mode for plain JSON consumers.
  const acceptHeader = req.headers && req.headers.accept;
  const acceptsEventStream = !!acceptHeader &&
    acceptHeader.split(',').some(t => {
      const parts = t.trim().split(';');
      if (parts[0].trim() !== 'text/event-stream') return false;
      const qParam = parts.find(p => p.trim().startsWith('q='));
      const q = qParam ? parseFloat(qParam.trim().slice(2)) : 1;
      return q > 0;
    });
  const returnEventStream = (format === 'event-stream') || acceptsEventStream;
...
HttpContext.prototype.respondWithEventStream = function(stream) {
  ...
  const toOutErr = (err) => {
    ...
    for (const k in err) o[k] = err[k];
    return o;
  };

  // Pause buffers data/end until SSE headers are written. Errors that fired
  // before this handler ran are stashed on stream._earlyError by the invoke
  // path; replay them after init.
  stream.pause();
  stream.on('error', (err) => client.send('error', toOutErr(err)));

  client.initialize().then(() => {
    if (stream._earlyError) client.send('error', toOutErr(stream._earlyError));
    stream.on('data', (chunk) => client.send('data', chunk));
    stream.on('end', () => client.send({event: 'end', data: 'null'}));
    if (stream.destroy) {
      client.once('close', () => stream.destroy());
    }
    stream.resume();
  }).catch(err => {
    console.warn('SSE client initialization failed:', err.message);
    stream.destroy();
  });
};
...
HttpContext.prototype.done = function(cb) {
  ...
  // text/event-stream is only meaningful for stream-typed methods; for
  // ordinary methods, exclude it so content negotiation picks the next
  // acceptable type (e.g. xml) rather than silently falling back to json.
  const negotiableTypes = this.supportedTypes.filter(
    t => t !== 'text/event-stream'
  );
  let accepts = this.req.accepts(negotiableTypes);
