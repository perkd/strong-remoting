...
const { enhanceRemoteObjects } = require('./promise-wrapper');
...
function RemoteObjects(options) {
  ...
  // Enhance with Promise support while maintaining 100% backward compatibility
  enhanceRemoteObjects(this);
}
...
/**
 * Disconnect from the remoting server and clean up connections.
 */
RemoteObjects.prototype.disconnect = function() {
  if (this.serverAdapter) {
    // Clean up the adapter if it has a disconnect method
    if (typeof this.serverAdapter.disconnect === 'function') {
      this.serverAdapter.disconnect();
    }

    // Clean up HTTP client connections if available
    if (this.serverAdapter.connection && this.serverAdapter.connection.destroy) {
      this.serverAdapter.connection.destroy();
    }
...
RemoteObjects.prototype.invoke = function(method, ctorArgs, args, callback) {
  ...
  return this.serverAdapter.invoke.apply(this.serverAdapter, arguments);
};
...
RemoteObjects.prototype.execHooks = function(when, method, scope, ctx, next) {
  ...
  function execStack(err) {
    ...
    if (cur) {
      try {
        // Enhanced hook execution supporting both callback and async patterns
        const hookResult = executeHookFunction(cur, scope, ctx, execStack, method);

        // If hook returned a Promise, handle it
        if (hookResult && typeof hookResult.then === 'function') {
          hookResult.then(
            function() { execStack(); }, // Success: continue to next hook
            function(hookErr) { next(hookErr); } // Error: stop execution
          );
        }
        // If hook was synchronous or callback-based, execStack was already called
      } catch (err) {
        next(err);
      }
    } else {
  ...
  // Enhanced hook function execution
  function executeHookFunction(hookFn, scope, ctx, execStack, method) {
    // Detect hook function signature
    const hookLength = hookFn.length;

    if (hookLength >= 2) {
      // Callback-style hook: function(ctx, next, method)
      debug('Executing callback-style hook with %d parameters', hookLength);
      return hookFn.call(scope, ctx, execStack, method);
    } else if (hookLength === 1) {
      // Potentially async hook: function(ctx) or async function(ctx)
      debug('Executing potential async hook');
      const result = hookFn.call(scope, ctx);
      ...
      if (result && typeof result.then === 'function') {
        ...
        // Synchronous hook that doesn't use callback
        debug('Hook executed synchronously');
        execStack(); // Continue immediately
        return null;
      }
    } else {
      // Zero-parameter hook (unusual but possible)
      debug('Executing zero-parameter hook');
      const result = hookFn.call(scope);
